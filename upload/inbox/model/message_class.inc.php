...
class message_class extends GZ_MODEL
{

	/**
	 * 用户发送短信息类
	 * @param unknown_type $sender_uid	发送用户ID
	 * @param unknown_type $recipient_uid	接收用户ID
	 * @param unknown_type $notice_title	短信息标题
	 * @param unknown_type $notice_content	短信息内容
	 * @param unknown_type $notice_type		短信息类型，0-普通消息10-系统发的消息，不能回复11-系统通知
	 * @param unknown_type $sender_del		发送者删除，默认0
	 * @param unknown_type $recipient_del	接收者删除，默认0
	 */
	public function send_message($sender_uid, $recipient_uid, $notice_title, $notice_content, $notice_type = 0, $sender_del = 0, $recipient_del = 0)
	{
		...
		//发送系统邮件通知
		$this->model('email')->message($sender_uid, $recipient_uid, $diag_id);
	 ...
	 * @param unknown_type $diag_id
	 * @param unknown_type $notice_title
	 * @param unknown_type $notice_content
	...
	public function add_dialog_unread($diag_id, $filed)
	{
		...
		$sql = "UPDATE " . $this->get_table('notice_dialog') . " SET " . implode(',', $update_arr) . " WHERE dialog_id = " . $diag_id;
	...
	public function read_message($dialog_id)
	{
		...
		if ($message_unread)
		{
			...
			//更新接收表时间
			$this->update('notice_recipient', array('recipient_time' => mktime()), 'recipient_uid = ' . $uid . ' AND dialog_id = ' . $dialog_id);
			
			//更新用户未读通知汇总数据
			$this->model('account')->increase_user_statistics(account_class::NOTICE_UNREAD, 0, $uid);
			
			ZCACHE::delete("inbox_unread_" . $uid);
		}
	...
	public function get_message_by_dialog_id($dialog_id, $limit = "0,10")
	{
		...
		$sql .= " WHERE a.dialog_id = " . $dialog_id . " AND ((b.sender_uid=" . $uid . " AND b.sender_del='0') OR (b.recipient_uid=" . $uid . " AND b.recipient_del='0')) ORDER BY notice_id DESC limit " . $limit;
		
		$rs["list"] = $this->query_all($sql);
		
		$rs["list_one"] = $this->query_all("SELECT * FROM " . $this->get_table('notice_dialog') . " WHERE dialog_id = " . $dialog_id);
	...
	{
		$sql = "SELECT COUNT(*) as count FROM " . $this->get_table('notice') . " as a LEFT JOIN " . $this->get_table('notice_recipient') . " as b ON a.notice_id = b.notice_id WHERE a.dialog_id = " . $dialog_id;
	...
	public function get_message_unread_num($uid, $dialog = 0)
	{
		...
		$where = "recipient_uid = " . $uid . " and recipient_time = 0 and recipient_del = 0";
		
		if ($dialog > 0)
		{
			$where .= " AND dialog_id = " . $dialog;
		}
	 ...
	 * @return array
	 */
	public function list_message($limit = "0,10")
	{
		$retval = array();
		
		$uid = USER::get_client_uid();
		
		$where = '((sender_uid = ' . $uid . ' AND sender_count > 0) OR (recipient_uid = ' . $uid . ' AND recipient_count > 0))';
		
		$list_msg = $this->fetch_all('notice_dialog', $where, 'last_time DESC', $limit);
		...
		if (! empty($list_msg[0]))
		{
			...
			{
				$diag_ids[] = $recordset['dialog_id']; //对话ID
				

				if ($uid == $recordset['recipient_uid']) //如为接收者，则查找发送者名称
				{
			...
			{
				$dia_info = $this->fetch_row('notice', 'dialog_id=' . $diag_id, 'notice_id DESC');
				$dia_info[0]['notice_content'] = nl2br(FORMAT::parse_links($dia_info[0]['notice_content']));
				$title_list[] = $dia_info[0];
			}
		}
		
		$retval['content_list'] = $list_msg; //汇总表数据
		$retval['user_list'] = $send_ids; //用户ID数组
		$retval['title_list'] = $title_list; //标题数组
	 ...
	 * @param  $limit
	 */
	public function list_not_read_message($uid,$limit = "0,10")	
	{
		$retval = array();
		
		$where = '(recipient_uid = ' . $uid . ' AND recipient_count > 0 AND recipient_unread = 1)';
		...
		if (! empty($list_msg[0]))
		{
			...
			foreach ($diag_ids as $diag_id)
			{
				...
				$dia_info['notice_content'] = nl2br(FORMAT::parse_links($dia_info['notice_content']));
	...
	public function count_user_message($uid)
	{
		return $this->count('notice_dialog', 'sender_uid = ' . $uid . ' OR recipient_uid = ' . $uid);
	}
	...
	public function check_send_message($uid)
	{
		...
		$user_focus = $this->model('follow')->get_user_friends($uid);
	...
	{
		$where = "(`sender_uid` = '" . $u_id . "' AND `recipient_uid` = '" . $s_id . "') ";
		
		$where .= " OR (`recipient_uid` = '" . $u_id . "' AND `sender_uid` = '" . $s_id . "' )";
		
		return $this->fetch_row('notice_dialog', $where);
	}
	...
	{
		$recipicent_info = $this->model('account')->get_users_by_uid($uid);
